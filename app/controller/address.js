...
exports.findAll = async (req, res) => {
    try {
        ...
        const searchFilter = q ? {
            $or: [
                { address1: { $regex: new RegExp(q, 'i') } },
                { state: { $regex: new RegExp(q, 'i') } },
            ]
        } : {}; 


        const address = await Address.find({ ...searchFilter, user_ID: userID })
        const totalCount = await Address.countDocuments()

        const getPaginationResult = await getPagination(req.query, address, totalCount);

        handleResponse(res, getPaginationResult, 'Address has been retrieved successfully.',200)
